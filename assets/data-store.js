(function () {
  ...
  function validateRelativeEntry(relativeKey, value) {
    ...
    if (kind === "text") {
      if (!/^p\d{2}\.[A-Za-z0-9._-]{1,160}$/.test(identifier) || value.length > MAX_TEXT_CHARACTERS) {
      ...
      if (!/^p\d{2}$/.test(identifier) || value !== "1") throw new TypeError("El estado de página no es válido.");
    } else if (kind === "settings") {
      if (identifier !== "issue" || value.length > 20_000) throw new TypeError("Los datos de edición no son válidos.");
      let settings;
      try {
